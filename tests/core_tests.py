...
from sanic_thumbnails.storage_backends import FilesystemStorageBackend
...
class CoreTestCase(unittest.TestCase):
    ...
    async def test_get_format(self):
        ...
        options = {'format': 'PNG'}
        self.assertEqual(await self.thumbnail._get_format(image, **options), 'PNG')

        options = {}
        self.assertEqual(await self.thumbnail._get_format(image, **options), self.app.config['THUMBNAIL_DEFAUL_FORMAT'])

    async def test_get_raw_data(self):
        image = Image.new('L', (10, 10))

        options = {'format': 'JPEG'}
        data = await self.thumbnail.get_raw_data(image, **options)
    ...
    async def test_create_thumbnail(self):
        ...
        new_image = await self.thumbnail._create_thumbnail(image, size=(50, 50))

        self.assertEqual(new_image.size, (50, 50))

        new_image = await self.thumbnail._create_thumbnail(image, size=(50, 50), crop=None)

        self.assertEqual(new_image.size, (50, 50))

    @mock.patch('sanic_thumbnails.utils.generate_filename')
    async def test_get_thumbnail(self, mock_thumb_name):
        with tempfile.NamedTemporaryFile(suffix='.jpg') as original:
            with tempfile.NamedTemporaryFile(suffix='.jpg') as thumb:
                ...
                thumb_url = await self.thumbnail.get_thumbnail(os.path.basename(original.name), '200x200')

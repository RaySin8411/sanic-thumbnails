...
class Thumbnail(object):
    ...
    async def thumbnail(self, img_url, size, crop=None, bg=None, quality=85):
        """
        ...
        miniature = await self._get_name(name, fm, size, crop, bg, quality)

        original_filename = os.path.join(self.app.config['MEDIA_FOLDER'], url_path, img_name)
        thumb_filename = os.path.join(self.app.config['MEDIA_THUMBNAIL_FOLDER'], 'cache', url_path, miniature)

        # create folders
        await self._get_path(thumb_filename)
        ...
        if os.path.exists(thumb_filename):
            ...
            if bg:
                img = await self._bg_square(img, bg)
    ...
    @staticmethod
    async def _bg_square(img, color=0xff):
        size = (max(img.size),) * 2
    ...
    @staticmethod
    async def _get_name(name, fm, *args):
        for v in args:
            if v:
                name += '_%s' % v
        name += fm

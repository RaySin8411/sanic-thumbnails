...
class Thumbnail(object):
    ...
    async def thumbnail(self, img_url, size, crop=None, bg=None, quality=100):
        """
        ...
        miniature = await _get_name(name, fm, size, crop, bg, quality)
    ...
    async def _bg_square(self, img, color=0xff):
        size = (max(img.size),) * 2
        layer = Image.new('L', size, color)
        layer.paste(img, tuple(map(lambda x: (x[0] - x[1]) / 2, zip(size, img.size))))
        return layer

    async def _get_path(self, full_path):
